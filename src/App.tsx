...
type Page = 'home' | 'workspace' | 'pricing' | 'login';
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f8faff] text-[#101a3a]">
      ...
      {page === 'workspace' ? <Workspace navigate={navigate} /> : page === 'pricing' ? <Pricing navigate={navigate} /> : page === 'login' ? <Login navigate={navigate} /> : <Home navigate={navigate} />}
      {page !== 'workspace' && page !== 'login' && <Footer navigate={navigate} />}
    </div>
  );
}
      ...
      <section className="hero-shell overflow-hidden">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-16 px-5 pb-24 pt-16 sm:px-8 lg:grid-cols-[.92fr_1.08fr] lg:pb-32 lg:pt-24">
          <div className="relative z-10 max-w-2xl">
            <div className="eyebrow"><span className="eyebrow-dot" /> AI background removal, made simple</div>
...
function Workspace({ navigate }: { navigate: (page: Page) => void }) { const [state, setState] = useState<UploadState>('empty'); const [file, setFile] = useState<File | null>(null); const [preview, setPreview] = useState(''); const [error, setError] = useState(''); const inputRef = useRef<HTMLInputElement>(null);
  const chooseFile = (candidate: File | undefined) => { if (!candidate) return; setError(''); if (!acceptedTypes.includes(candidate.type)) { setError('Please choose a JPG, PNG, or WEBP image.'); return; } if (candidate.size > maxFileSize) { setError('That image is larger than 10 MB. Choose a smaller file to continue.'); return; } setFile(candidate); setPreview(URL.createObjectURL(candidate)); setState('selected'); };
  const onDrop = (event: DragEvent<HTMLDivElement>) => { event.preventDefault(); chooseFile(event.dataTransfer.files[0]); };
  const reset = () => { setFile(null); setPreview(''); setError(''); setState('empty'); if (inputRef.current) inputRef.current.value = ''; };
  const process = () => { if (!file) return; setState('processing'); window.setTimeout(() => setState('result'), 1700); };
...
function Result({ preview, reset, navigate }: { preview: string; reset: () => void; navigate: (page: Page) => void }) {
